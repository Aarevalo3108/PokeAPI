...
const Pokemon = (data = {url}) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-center items-center gap-6 border-r-2 border-b-2 border-gray-900 rounded-lg bg-gray-700 drop-shadow hover:scale-110 p-4">
      <div className="flex flex-col w-2/3">
        <h1 className="capitalize bg-green-800 text-gray-200 rounded-lg mt-2 px-2">{poke.name} #{poke.id}</h1>
        <img className="h-32 w-32 m-4" src={poke.img} alt={poke.name || 'pokemon'} />
        <span className="flex gap-2 justify-center items-center">
          {poke.types && poke.types.map((type, index) => (
            <img className="max-w-18 min-w-16 h-4" key={index} src={Types(type.type.name)} alt={type.type.name}></img>
            ))
          }
        </span>
      </div>
      <div className="flex sm:flex-col sm:w-1/3 gap-4 sm:gap-2">
        <div>
          <img src={Heart} alt="logo"  className='w-12 h-12 opacity-20'/>
